...
/* private method */
ssize_t network::recvfrom_ovl(int fd, uint8_t* buffer, size_t count, int flags, struct sockaddr* addr, socklen_t* addr_len){
  return recvfrom(fd, buffer, count, flags, addr, addr_len);
}
ssize_t network::recvfrom_ovl(int fd, uint16_t* buffer, size_t count, int flags, struct sockaddr* addr, socklen_t* addr_len){
  size_t temp_counter = 0;
  ssize_t rf_retnum = 0;
  int rf_errno = 0;

  rf_retnum = recvfrom(fd, buffer, count*sizeof(uint16_t), flags, addr, addr_len);
  rf_errno = errno;
  if (rf_retnum < 0) {
    return -1*rf_errno;
  }
...
ssize_t network::recvfrom_ovl(int fd, uint32_t* buffer, size_t count, int flags, struct sockaddr* addr, socklen_t* addr_len){
  size_t temp_counter = 0;
  ssize_t rf_retnum = 0;
  int rf_errno = 0;

  rf_retnum = recvfrom(fd, buffer, count*sizeof(uint32_t), flags, addr, addr_len);
  rf_errno = errno;
  if (rf_retnum < 0) {
    return -1*rf_errno;
  }
...
int network::common_setup(const std::string& com_ip, const std::string& com_port){
  nw_connected = false;
  ...
  accept_pollfd.events = POLLRDNORM | POLLRDHUP | POLLHUP | POLLERR | POLLNVAL;
#else
...
/* public method */
network::network(const std::string& con_ip_addr, const std::string& con_port, const int con_family, const int con_sock_type){
...
network::network() : network("127.0.0.1", "62341", AF_INET, SOCK_DGRAM){}
network::network(const std::string& con_ip_addr, const std::string& con_port) : network(con_ip_addr, con_port, AF_INET, SOCK_DGRAM){}
...
*/
int network::set_address(const std::string& dest_ip, const std::string& dest_port){
  if(nw_connected) {
    close(socket_fd);
    nw_connected = false;
  }
  addr_info_hint.ai_family = AF_INET;
  addr_info_hint.ai_socktype = SOCK_DGRAM;
...
/*
  ...
  return:
    ...
    other -> error(return value of getaddrinfo())
*/
int network::set_address(const std::string& dest_ip, const std::string& dest_port, const int con_family, const int con_sock_type){
  if (nw_connected) {
    close(socket_fd);
    nw_connected = false;
  }
  ...
  for (struct addrinfo* di_ref = dest_info; di_ref != nullptr; di_ref = di_ref->ai_next) {
    ...
    if (socket_fd != -1) {
      if (!is_blocking) {
        ...
        } while ((errno_ret == EINPROGRESS) || (errno_ret == EALREADY) && !network_force_return_req);
